...
- [x] **1.1** Создать структуру модуля `src/curation/` с `__init__.py`
- [x] **1.2** Реализовать `ThumbnailGenerator` в `src/curation/thumbnail.py`
  - Resize с сохранением aspect ratio (max 512x512)
  - JPEG compression (quality 60)
  - Base64 encoding для API
- [x] **1.3** Написать unit-тесты для ThumbnailGenerator
  - Тест: выходной размер ≤ 512x512
  - Тест: выходной файл ≤ 50KB
  - Тест: различные форматы входа (jpg, png, webp)
- [x] **1.4** Добавить Pillow в зависимости проекта
...
- [x] **3.1** Реализовать `CurationPipeline` в `src/curation/pipeline.py`
  - Поиск изображений в папке
  - Threshold-based filtering
  - Перемещение в `data/curated/`
- [x] **3.2** Добавить структуру `data/curated/` в проект
- [x] **3.3** Реализовать dry-run режим
- [x] **3.4** Добавить reporting/logging
  - Результат по каждому изображению
  - Итоговая статистика
- [x] **3.5** Написать integration тесты
...
- [x] **4.1** Создать CLI в `src/curation/cli.py` с командами:
  - `curate <path>` — курация папки
  - `curate-all` — курация всего incoming
  - `stats` — статистика
- [x] **4.2** Добавить entry point в `pyproject.toml`
- [x] **4.3** Добавить документацию в README или docs/
- [x] **4.4** Финальное тестирование на реальных данных
...
| `langchain-groq` | Groq API | Installed |
| `typer` | CLI | Installed |
